...
public class MyShopList_07 {
		...
		try {
			...
			while (true) {
				...
				*/
				...
				//방법2.
				MyShop s= new MyShop(sang,su,price,total); //명시적 생성후
				s.setTotal(total);
				
				//리스트에 추가
				list.add(s);
			}
			System.out.println("파일에서 총 "+list.size()+"개의 상품목록을 가져왔어요");
	...
	public void shopFileWrite() {
		...
		try {
			...
			for (MyShop sp : list) {
				String s = sp.getSangpum() + "," + sp.getSu() + "," + sp.getPrice() + ","+sp.getTotal()+"\n";
				fw.write(s);
			}
	...
	public void shopAdd() {
		...
		//MyShop 클래스가 생성
		MyShop sp = new MyShop(sang, su, price,total);
		
		//list에 sp 추가
		list.add(sp);
		System.out.println("***상품정보가 입력되었습니다!***");
	}
	...
	public void shopAllDatas() {
		...
		int no =1;
		for(MyShop sp : list) {
			System.out.println(no++ + "\t" + sp.getSangpum()+ 
					"\t" +sp.getSu()+ "\t" + sp.getPrice() + 
					"\t" + sp.getTotal());
		}
	}
	public void shopDelete() {
		...
		for(int i = list.size()-1; i>=0; i--) {
			MyShop sp =list.get(i);
			
			if(sp.getSangpum().equals(sang))
			{
				flag =true;
				list.remove(i);
			}
	...
	public void shopSearch() {
		...
		for(MyShop sp :list)
		{
			if(sp.getSangpum().contains(sang))
			{
				flag = true;
				
				System.out.println("상품명: "+ sp.getSangpum());
				System.out.println("수량: "+ sp.getSu());
				System.out.println("가격: "+ sp.getPrice());
				System.out.println("총금액: "+sp.getTotal());
				System.out.println("----------------------------------");
			}
		}
		if(!flag)
	...
	public void process() {
		...
		while(true)
		{
			...
			if(num == 1)
				...
				System.out.println("Shop정보를 저장후 종료할게요");
				this.shopFileWrite();
				System.exit(0);
			}else {
	...
	public static void main(String[] args) {
		...
		MyShopList_07 frw = new MyShopList_07();
		frw.process();
	}
